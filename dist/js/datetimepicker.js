				...
				timeboxparent
					...
					.on("scroll_element.xdsoft_scroller", function (event, percent) {
						...
						setTimeout(function () {
							timebox.css(
								"marginTop",
								-parseInt((timebox[0].offsetHeight - parentHeight) * percent)
							);
						}, 10);
					})
		...
		var KEY0 = 48,
			...
			createDateTimePicker = function (input) {
				var datetimepicker = $(
					...
					(options.weeks ? " xdsoft_showweeks" : "") +
					options.className +
					'"></div>'
				),
					xdsoft_copyright = $(
						...
						input
							...
							.on("blur.xdsoft", function () {
								if (options.allowBlank && !$(this).val().trim().length) {
									...
									$(this).val(
										_xdsoft_datetime.now().dateFormat(options.format)
									);
					...
					_this.strToDateTime = function (sDateTime) {
						...
						if (
							...
							currentTime = new Date(
								_xdsoft_datetime.now().getTime() +
								parseInt(tmpDate[1] + "1") * timeOffset
							);
						} else
